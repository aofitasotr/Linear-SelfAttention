...
from training.synthetic_pipeline import run_synthetic_k_sweep, train_synthetic_model


def build_arg_parser(add_help: bool = True) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Обучение на синтетической задаче Positional Lookup",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        add_help=add_help,
    )
    parser.add_argument("--mode", type=str, default="single", choices=["single", "k_sweep"])
    parser.add_argument("--attention_type", type=str, default="pos-enc", choices=SYNTHETIC_ATTENTION_TYPES)
    ...
    parser.add_argument("--k_values", type=str, default="1,2,3,4,5,6,7,8")
    ...
    parser.add_argument("--log_dir", type=str, default="./synthetic_logs")
    parser.add_argument("--results_csv", type=str, default="./synthetic_results.csv")
...
def main(argv=None):
    ...
    k_values = [int(value.strip()) for value in args.k_values.split(",")]
    ...
    if args.mode == "single":
        ...
        write_log("\n" + "=" * 70)
        write_log("РЕЗУЛЬТАТЫ")
        write_log("=" * 70)
        write_log(f"Модель: {'Оригинальная' if args.use_original_model else args.attention_type}")
        write_log(f"k: {args.k}")
        write_log(f"Accuracy: {results['test_accuracy']:.4f}")
        write_log(f"F1-macro: {results['test_f1_macro']:.4f}")
        write_log(f"Time per sample: {results['test_time_per_sample_ms']:.2f} мс")
        write_log("=" * 70)
        return

    df = run_synthetic_k_sweep(
    ...
    write_log("\n" + "=" * 70)
    write_log("K-SWEEP РЕЗУЛЬТАТЫ")
    write_log("=" * 70)
    write_log(df[["attention_type", "k", "test_accuracy", "test_f1_macro"]].to_string())
    write_log("=" * 70)
